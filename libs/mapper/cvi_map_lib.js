/**
 * cvi_map_lib.js 2.2 (10-Jun-2008)
	...
	modify: function(image, options) {
		try {		
				...
				for(var j=0;j<map.areas.length;j++) {
					if(map.areas[j].shape.match(/(rect|poly|circle)/i)) {
						if(window.opera||map.areas[j].coords!='') {
							...
							if(isVM||isIE) {
								func = map.areas[j].onmouseover; if(func!=null) {tmp=String(func); func=tmp.substr(23,tmp.length-25);}
								map.areas[j].onmouseover = new Function('setAreaOver(this,"'+canvas.id+'","'+bcolor+'","'+acolor+'",'+opacity+','+noborder+','+nofade+','+image.jg+');'+func); 
								func = map.areas[j].onmouseout; if(func!=null) {tmp=String(func); func=tmp.substr(23,tmp.length-25);}
						...
						for(var j=0;j<map.areas.length;j++) {
							if(map.areas[j].shape.match(/(rect|poly|circle)/i)) { 
								if(window.opera||map.areas[j].coords!='') {
									atr = map.areas[j].id;
									if(isVM||isIE) {
										func = map.areas[j].onmousemove; if(func!=null) {tmp=String(func); func=tmp.substr(23,tmp.length-25);}
			...
			if(canvas && bgrnd && map) {
				...
				if(isVM||isIE) {
					func = image.onmousemove; if(func!=null) {tmp=String(func); func=tmp.substr(23,tmp.length-25);} image.movefunc = func;
				}else {
